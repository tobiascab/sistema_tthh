...
// Context para compartir el estado activeTab entre LoginForm y LoginPage
const LoginTabContext = createContext<{
    activeTab: 'admin' | 'colaborador';
    setActiveTab: (tab: 'admin' | 'colaborador') => void;
}>({
    activeTab: 'admin',
    setActiveTab: () => { },
});

export const useLoginTab = () => useContext(LoginTabContext);

export default function LoginPage() {
    const [activeTab, setActiveTab] = useState<'admin' | 'colaborador'>('admin');

    return (
        <LoginTabContext.Provider value={{ activeTab, setActiveTab }}>
            <div className="min-h-screen flex items-center justify-center bg-[#E8F5E9] relative overflow-hidden p-4">
                ...
                >
                    ...
                    {/* Left Side - Form (usa layout para animación automática) */}
                    <motion.div
                        layout
                        key="form-panel"
                        className="w-full md:w-1/2 p-8 md:p-12 lg:p-20 flex flex-col justify-center relative order-1 md:order-none"
                        style={{
                            order: activeTab === 'admin' ? 1 : 2
                        }}
                        initial={false}
                        transition={{
                            layout: {
                    ...
                    {/* Right Side - Visual / Branding (usa layout para animación automática) */}
                    <motion.div
                        layout
                        key="visual-panel"
                        className="hidden md:flex w-1/2 relative items-center justify-center overflow-hidden order-2 md:order-none"
                        style={{
                            order: activeTab === 'admin' ? 2 : 1
                        }}
                        initial={false}
                        transition={{
                            layout: {
