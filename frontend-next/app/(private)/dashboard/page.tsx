...
function AdminDashboard() {
    ...
    const metricsConfig = [
        ...
        {
            title: "Tasa de Formación",
            value: (dashboardData?.horasFormacionMes || 0) + " hs",
            subtitle: "Capacitación este mes",
            icon: GraduationCap,
...
function ColaboradorDashboard() {
    ...
    const { data: cursos, isLoading: loadingCursos } = useQuery({
        queryKey: ["capacitaciones-disponibles"],
        queryFn: () => capacitacionesApi.getAll(),
    });

    const solicitudesPendientes = ausencias?.content?.filter(a => a.estado === "PENDIENTE")?.length || 0;
    const cursosProximos = cursos?.filter(c => c.estado === "ACTIVA" && c.cupoDisponible > 0)?.slice(0, 2) || [];
        ...
        <div className="space-y-8 pb-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {[
                    ...
                    { title: "Campus Virtual", sub: "Ver cursos y talleres", icon: GraduationCap, href: "/tthh/capacitaciones", color: "text-blue-600", bg: "bg-blue-50" },
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                {/* Cursos Promocionados */}
                <motion.div initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.4 }}>
                    <Card className="bg-neutral-900 border-none shadow-xl text-white h-full relative overflow-hidden rounded-2xl">
                        <div className="absolute top-0 right-0 w-64 h-64 bg-emerald-600/20 rounded-full blur-3xl -translate-y-10 translate-x-10"></div>
                        <CardHeader>
                        ...
                        <CardContent className="relative z-10">
                            <div className="space-y-4">
                                {loadingCursos ? (
                                    <div className="space-y-2">
                                        {[1, 2].map(i => <Skeleton key={i} className="h-20 w-full bg-white/10 rounded-xl" />)}
                                    </div>
                                ) : cursosProximos.length > 0 ? (
                                    <>
                                        {cursosProximos.map(curso => (
                                            <div key={curso.id} className="p-4 bg-white/5 border border-white/10 rounded-xl hover:bg-white/10 transition-colors">
                                                <div className="flex justify-between items-start mb-1">
                                                    <h4 className="font-bold text-sm">{curso.nombreCapacitacion}</h4>
                                                    <Badge className="bg-emerald-500 text-white border-none text-[10px]">
                                                        {curso.cupoDisponible} cupos
                                                    </Badge>
                                                </div>
                                                <div className="flex gap-3 text-xs text-neutral-400 mt-2">
                                                    <span>📅 {format(new Date(curso.fechaInicio), "dd MMM", { locale: es })}</span>
                                                    <span>⏱️ {curso.duracionHoras}hs</span>
                                                </div>
                                            </div>
                                        ))}
                                        <Link href="/tthh/capacitaciones" className="block pt-2">
                                            <Button size="sm" className="w-full bg-emerald-600 hover:bg-emerald-500 text-white border-none">
                                                Explorar Catálogo
                                            </Button>
                                        </Link>
                                    </>
